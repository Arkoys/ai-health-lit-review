...
Extraction was performed by the single author of this review, acting in
both extractor and verifier roles.
...
\textbf{Framework emergence.} The three analytical dimensions of the
resulting framework --- ecological validity, governance capacity, and
the pilot trap --- were \textit{not} defined a priori. They emerged
iteratively during the coding process through repeated convergence of
codes across the evaluation, governance, and implementation domains of
the included studies. Once stable, these dimensions were then used as
an organizing lens for subsequent quantification and comparison. This
emergent, inductive grounding is important: the framework is reported
as an analytical synthesis \textit{derived from} the literature, not
as a hypothesis \textit{imposed upon} it.

\textbf{Inter-rater reliability via multi-pass LLM coding replication.}
Given the single-researcher design of this review, inter-rater
reliability was approximated using multi-run LLM-based coding
replication with varied prompting strategies to simulate independent
annotators. This provides a lower-bound estimate of coding robustness
rather than a formal human inter-rater agreement measure.
