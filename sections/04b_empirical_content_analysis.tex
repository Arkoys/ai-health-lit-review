...
\begin{figure}[t]
  ...
  %   \includegraphics[width=0.85\textwidth]{analysis/figures/vicious_cycle.png}
  \fbox{\parbox[c][6cm][c]{0.85\textwidth}{\centering
    ...
    pathway emphasised.]}}}
  \caption{Conceptual schematic of the three-gap framework as a
  self-reinforcing vicious cycle. The three gaps are mutually
  reinforcing: ecological validity failures (gap1) erode the
  evidence base on which governance capacity (gap2) is built;
  weak governance allows the pilot trap (gap3) to persist;
  unevaluated pilots generate no feedback loop to correct
  upstream evaluation practices. The empirical pathway analysis
  above (Figure~\ref{fig:pathways}) identifies gap1 $\rightarrow$
  gap3 as the dominant directional pathway, but the cycle view
  shown here emphasises that each gap can act as either cause or
  consequence depending on the deployment context.}
...
\begin{itemize}
  ...
  \item \textbf{The gaps co-occur} in 30 of 50 gap-positive
        ...
        is consistent with the cascade interpretation under
        which G1 and G3 are strongly coupled (co-occurrence of
        G1$\wedge$G3 = 12 papers) while the all-three
        configuration is the limit case of the cascade.
