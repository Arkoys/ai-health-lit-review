...
\code{~/.hermes/.env} at runtime, \emph{not} the project's \texttt{.env}):
...
\code{reporter.py} and handles its own retry logic. The Hermes gateway's
Telegram integration (defined in \code{~/.hermes/config.yaml}) manages the
...
through the Hermes cron system (\code{mcp\_cronjob}):
...
\code{deliver: origin}, routing output back to the originating Telegram DM.
The \code{--push} flag ensures outputs are committed to GitHub before the job
completes.
...
\code{providers} key. The primary provider is OpenRouter
